...
public class ui_change : MonoBehaviour {
    ...
    */
    ...
    {
        if (exp_type == "C/DC") {
            ExportToExcel(end_time.dt, "D:\\正式实验数据\\2D实验数据\\CDC.xls");
        }
        if (exp_type == "D/AC")
        {
            ExportToExcel(end_time.dt, "D:\\正式实验数据\\2D实验数据\\DAC.xls");
        }
        if (exp_type == "A/DP")
        {
            ExportToExcel(end_time.dt, "D:\\正式实验数据\\2D实验数据\\ADP.xls");
        }
        if (exp_type == "D/DC")
        {
            ExportToExcel(end_time.dt, "D:\\正式实验数据\\2D实验数据\\DDC.xls");
        }
        if (exp_type == "C/AC")
        {
            ExportToExcel(end_time.dt, "D:\\正式实验数据\\2D实验数据\\CAC.xls");
        }
